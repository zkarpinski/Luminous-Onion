...
test("FindingSummarySmall renders successfully with each Severity Type and Value", () => {
  const { container } = render(
    <BrowserRouter>
      <FindingSummarySmall {...mockFindingData} />
    </BrowserRouter>,
  );
...
test("FindingSummarySmall renders disabled", () => {
  const { container } = render(
    <BrowserRouter>
      <FindingSummarySmall {...mockFindingData} disabled />
    </BrowserRouter>,
  );

  expect(container.getElementsByClassName("SeverityTileDisabled").length).toBe(
    5,
  );
});
